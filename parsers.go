...
const (
	...
	gtNoFiles = `^\?.*\[no test files\]$`
	// FAIL    node/config [build failed]
	gtBuildFailed = `^FAIL.*\[(build|setup) failed\]$`
...
func gtParse(rd io.Reader) ([]*Suite, error) {
	...
	isNofiles := regexp.MustCompile(gtNoFiles).MatchString
	isBuildFailed := regexp.MustCompile(gtBuildFailed).MatchString
